...
tin_range=spTransform(tin_range,CRS(projection(points)))

## plot it again with the range
spplot(points,zcol="type",pch=1:4,col.regions=c("red","red","black","red"))+
...
hist(points$dist)
## so some points are > 2,000km from the range, let's drop any more than 10,000km
...
xyplot(npp~alt,groups=obs,data=pointsd@data,auto.key=T)
xyplot(tmean12~prec12,groups=obs,data=pointsd@data,auto.key=T)

## Fit a very simple GLM to the data
m1=glm(obs~tmean12+prec12+npp+alt,data=pointsd@data)
